  ...
  def retrieve(user_query: str) -> List[Dict]:
      ...
      # Use threshold of 2 so client queries with small datasets still stay filtered
  ...
  - If the context does not contain enough information to answer, say exactly: "No specific data available in current collateral. Check 
  ...
  - If no chunk has a "Client:" field with a real value (i.e. all show "N/A"), say: "No specific client data available in current       
  collateral. Check with the delivery team."
  ...
  def stream_answer(
      ...
      if user_query.strip().lower().rstrip("!.,") in _GREETINGS:
          yield "Hello! How can I help you with MindMap Digital's sales collateral? You can ask about case studies, capabilities, ROI   
  metrics, or specific industry use cases."
          return
